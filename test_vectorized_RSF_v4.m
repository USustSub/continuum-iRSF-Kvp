%% Clean version 
% % scp -r  ~//Downloads/Test3_2/*.m goudarzi@eejit.geo.uu.nl:/quanta1/home/goudarzi/Run2/
...
mu_gp = sin_phi_ ; 
% eta_vp = 0*2.5e15;       
if strcmp(Ex,'Herr') || strcmp(Ex,'Preuss')
...
%%
            ...
            while (normmm)>1e-16
                ...
                [sin_phi_0,Vp,Eii_p,theta_new] = get_mu_new ...
                    ( dg_2 , dQdsigma_ , dt , inx , deltaY/2 , b_ , theta_ , Mat_);
                ...
                if inc == 1
                    [sin_phi_hhh,~,~,~] = get_mu_new ( (dg_2+hhhh) , dQdsigma_ , dt , inx , deltaY/2 , b_ , theta_ , Mat_);
                    [sin_phi_hhh2,~,~,~] = get_mu_new ( (dg_2+i*hhhh) , dQdsigma_ , dt , inx , deltaY/2 , b_ , theta_ , Mat_);
...
bb_ = [b1_:0.001:b2_];
bb_ = b2_ ; 
Vp = [1e-30 1e-25 1e-20 1e-15 1e-10 1e-5 ]
...
%%
LD1 = dlmread('~/Desktop/Run_cluster/out/LD.log');
LD2 = dlmread('~/Downloads/Test3/out_5/LD.log');
figure
