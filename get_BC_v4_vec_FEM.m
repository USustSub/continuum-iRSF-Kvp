...
% % % % %         Shape2{cc_ij2}{2} = en ;  
% % % % %         Shape2{cc_ij2}{3} = phi ;
if norm(node_c(1)-0)<1e-5 ||  norm(node_c(1)-L)<1e-5 || norm(node_c(2)-0)<1e-5 || norm(node_c(2)-D)<1e-5
        [phi,B,en] = get_data_BC ( node_c , node , di , form ) ;
...
elseif strcmp(Ex,'Slip') || strcmp(Ex,'Herr') || strcmp(Ex,'Junction') || strcmp(Ex,'Preuss') || strcmp(Ex,'Preuss2')
     if norm(node_c(1)-0)<1e-5 % left edge
            if (inc == 1 & plast_it==1)  
                DD_BC = [DD_BC ; node_c ] ; 
% set uy = 0 
                K_BC_(2*ij-0,en(2:2:end)) =  phi ;
            end
                F(2*ij-0,1) = F(2*ij-0,1) + 0 ;
% set duxdx = 0 
                if (inc == 1 & plast_it==1)  
                        dphidx = B(1,1:2:end) ;
                    if node_c(2) > 0 & node_c(2) < D
%                         K_BC_(2*ij-1,en(1:2:end)) =  dphidx ;
                    end
                end
