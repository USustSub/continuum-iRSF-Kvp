...
maxS,SS,SS0,id1,Bx,By,En,Shape,Gve_,K__,m_type] = Pre_process_new_efficient ()
...
% % %                     HH(ij,ii*6-0) = C0 ; 
% % %                 end
% % %                 if norm(abs(xx(1)-L/2))<=dY/2+0.01*dY % && xx(1) > 0.2 && xx(1)<0.8
% % %                     DD = [ DD ; xx ij+(ii-1)*numnode] ; 
% % %                     C0 = 0.00 ; 
% % %                     HH(ij,ii*6-0) = C0 ; 
% % %                 end
...
%                 lines_ = [ 0.2 0.1 ; 0.5 0.3 ; 0.4 0.6 ; 0.8 0.3 ] * L ;
                load lines_.mat
                lines_ = lines_*L ; 
                
                for ij_ = 1 :size(lines_,1)/2
%                     val_ = L/200 ;             
                    val_ = fault_width/1 ; 
