...
AP_0 = 0 ; S0 = 0 ; 
uy = 2e-9 ;
r1 = find(abs(node(:,1)-L)<1e-5);
r2 = find(abs(node(:,1)-(L-dY))<1e-5);
l1 = find(abs(node(:,1)-0)<1e-5);
l2 = find(abs(node(:,1)-(0+dY))<1e-5);
% % figure
% % plot(node(r1,1),node(r1,2),'sq-')
% % hold on
% % plot(node(r2,1),node(r2,2),'sq-')
% % plot(node(l1,1),node(l1,2),'sq-')
% % plot(node(l2,1),node(l2,2),'sq-')
...
% mean(vu0(topNodes(:)*2-1))
% mean(au0(topNodes(:)*2-1))
...
% plot(SS0(inx,1),sin_phi_(inx,1),'bsq')
% hold on
            hhhh = 0.0000000000000001 ;
            ittt = 0 ; 
            while (normmm)>1e-16
                ittt = ittt + 1; 
%                 Update_mu  ; 
                [sin_phi_0,Vp,Eii_p,theta_new] = get_mu_new ( dg_2 , dQdsigma_ , dt , inx , deltaY/2 , b_ , theta_ );
...
%                 res1 = 0 - sqrt(J2_) + P_.*sin_phi_hhh + (dg_2+hhhh).*Gve_ + (dg_2+hhhh).*K__.*sin_phi_hhh.*sin_psi + (dg_2+hhhh)*eta_vp/dt ;%+ min(0,1e14*(dg_2+hhhh));
                ...
                if inc == 1
                    [sin_phi_hhh,~,~,~] = get_mu_new ( (dg_2+hhhh) , dQdsigma_ , dt , inx , deltaY/2 , b_ , theta_ );
                    [sin_phi_hhh2,~,~,~] = get_mu_new ( (dg_2+i*hhhh) , dQdsigma_ , dt , inx , deltaY/2 , b_ , theta_ );
...
%                 plot(SS0(inx,1),P_(inx,1).*sin_phi_(inx,1),'bsq')
%                 plot(SS0(inx,1),(res2(inx,1)),'rsq')
%                 plot(SS0(inx,1),P_(inx,1).*sin_phi_(inx,1),'bsq')
%                 plot(SS0(inx,1),dg_2(inx),'ksq')
%               plot(SS0(inx,1),sqrt(J2_(inx,1))-P_(inx,1).*sin_phi_(inx,1),'rsq')
%                 pause%(1)
            end
                if ittt > 10
                    warning('ittt>10')
...
close all
    figure
hold on
length(find(dg_c(inx)<0))
for iii_  = 440%1:length(inx)
%     if dg_c(inx(iii_))<0
        disp('h')
     ccc = 0 ; 
    DD_ = [-0.00009*0:0.0000000001:0.0000009*2] ; 
    res2 = zeros(length(DD),1');
    for dg_2 = DD_
        ccc = ccc + 1 ;
%         dg_2
                [sin_phi_0,Vp,Eii_p,theta_new] = get_mu_new2 ( dg_2 , dQdsigma_(inx(iii_),:) , dt , inx(iii_) , deltaY/2 , b_( inx(iii_)) , theta_( inx(iii_)) );
                sin_phi_ = sin_phi_0  ;
                res2(ccc) = 0 - sqrt(J2_(inx(iii_))) + P_(inx(iii_)).*sin_phi_ + dg_2.*Gve_(inx(iii_))  + dg_2*eta_vp/dt ; 
    ...
    plot(DD_,(res2),'r.-')
    hold on
    plot([min(DD_) max(DD_)],[0 0],'k-')
    plot(dg_c(inx(iii_)),resc(inx(iii_)),'bsq-')
    pause(0.001)
%     end
end
...
plot(SS0(:,1),SS0(:,2),'r.')
hold on
plot(SS0(inx(iii_),1),SS0(inx(iii_),2),'bsq')
plot([0 L],[D/2 D/2],'k-')
%}
            %%
%{
...
%                     dpar
    ...
    %                 Update_mu;
                    [sin_phi_,mu_gp] = get_mu_new ( dg_2 , dQdsigma_ , dt , inx , deltaY , b_ , theta_n);
...
%             dpar
        ...
        %% 
                ...
                dg_2 = 0*dg_2; 
                [sin_phi_,~] = get_mu_new ( dg_2 , dQdsigma_ , dt , inx , deltaY , b_ , theta_n);
                [sin_phi_hhh,~] = get_mu_new ( (dg_2+hhhh) , dQdsigma_ , dt , inx , deltaY , b_ , theta_n);
                res1 = 0 - sqrt(J2_) + P_.*sin_phi_hhh + (dg_2+hhhh).*Gve_ + (dg_2+hhhh).*K__.*sin_phi_hhh.*sin_psi + (dg_2+hhhh)*eta_vp/dt ;%+ min(0,1e14*(dg_2+hhhh));
                res2 = 0 - sqrt(J2_) + P_.*sin_phi_ + dg_2.*Gve_ + dg_2.*K__.*sin_phi_.*sin_psi + dg_2*eta_vp/dt ; %+ min(0,1e14*(dg_2));
        %         J = (Res(dg_+hhhh)-Res(dg_))/hhhh;
                J = (res1-res2)/hhhh; 
...
%             norm(sqrt(J2_2(inx)) -  ( sqrt(J2_(inx)) - Gve*dg_(inx) ) ) 
...
%             K_BC_(r1*2-1,r1*2-1) = 1;
%             K_BC_(r1*2-1,r2*2-1) = -1 ;
%             K_BC_(l1*2-1,l1*2-1) = 1;
%             K_BC_(l1*2-1,l2*2-1) = -1 ;
%             for ib_ = 1 : length(r1)
%                  K_BC_(r1(ib_)*2-1,r1(ib_)*2-1)=1;
%                  K_BC_(r1(ib_)*2-1,r2(ib_)*2-1)=-1;
%                  K_BC_(l1(ib_)*2-1,l1(ib_)*2-1)=1;
%                  K_BC_(l1(ib_)*2-1,l2(ib_)*2-1)=-1;
%             end
...
%         pause
    ...
    dlmwrite(['out/LD.log' ],[LD(:,1) LD(:,2) LD(:,3) LD(:,4) ],'Delimiter',' ')            
...
%         sin_phi_ = get_mu_new ( dg__ , dQdsigma__ , dt , inx , deltaY , b_ , theta_n );
